...
int main(int argc, char* argv[])
{
   ...
   // auto nsweeps = input.getInt("nsweeps");
    ...
    std::string runlogfile="runlog_k1c_";
    ...
    if(readmps) {
        ...
        for (int j = 1; j <= (lpbc ? N : N-2); j += 2) // PBC/OBC, channel 1 hopping
        {
             ampo += -t, "Cdagup", (j+1)%N+1, "Cup", j;
             ampo += -t, "Cdagup", j,         "Cup", (j+1)%N+1;
             ampo += -t, "Cdagdn", (j+1)%N+1, "Cdn", j;
             ampo += -t, "Cdagdn", j,         "Cdn", (j+1)%N+1;
        }

        for (int j = 1; j < N; j += 2) // channel 1 Kondo coupling
        {
        ...
        for (int j = 2; j <= (lpbc ? N : N-2); j += 2) // PBC/OBC, spin Heisenberg term
        {
            ampo += 0.5 * Jh, "S+", j, "S-", (j+1)%N+1;
            ampo += 0.5 * Jh, "S-", j, "S+", (j+1)%N+1;
            ampo +=       Jh, "Sz", j, "Sz", (j+1)%N+1;
        }
    ...
    if(domeas){
        ...
        for (int j = 1; j <= (lpbc ? N : N-2); j += 2) // PBC/OBC, channel 1 hopping
        {
             ampo_ek += -t, "Cdagup", (j+1)%N+1, "Cup", j;
             ampo_ek += -t, "Cdagup", j,         "Cup", (j+1)%N+1;
             ampo_ek += -t, "Cdagdn", (j+1)%N+1, "Cdn", j;
             ampo_ek += -t, "Cdagdn", j,         "Cdn", (j+1)%N+1;
        }
        ...
        for (int j = 2; j <= (lpbc ? N : N-2); j += 2) // PBC/OBC, spin Heisenberg term
        {
            ampo_ejh += 0.5 * Jh, "S+", j, "S-", (j+1)%N+1;
            ampo_ejh += 0.5 * Jh, "S-", j, "S+", (j+1)%N+1;
            ampo_ejh +=       Jh, "Sz", j, "Sz", (j+1)%N+1;
        }
