...
main()
    ...
    for (int j = 1; j <= N/2-1 ; j += 1) //electron hopping 
        {
            ampo += -t, "Cdag", j+3, "C", j;
            ampo += -t, "Cdag", j,   "C", j+3;
        }
    for(int j = 3; j <= N/2-1; j += 3) // electron hopping over orbitals
        {
            ampo += -J, "Cdag", j-2, "C", j;
            ampo += -J, "Cdag", j,   "C", j-2;
            ampo += -J, "Cdag", j+1, "C", j;
    ...
    for (int j = 1; j <= N/2  ; j += 3) //onsite V 这里几项我尝试配做particle-hole对称的形式，去掉了一个常数项
        {
            ampo += V, "Cdag", j, "C", j;
    }
    ...
    // electron pairing
    for (int j = 1; j <= N/2-1 ; j += 1) //electron pairing
        {
    ...
    //RIGHT HALF
    for (int j = N/2 + 2 ; j <= N-3 ; j += 1) //electron hopping 
    {
        ampo += -t, "Cdag", j+3, "C", j;
        ampo += -t, "Cdag", j,   "C", j+3;
    }
    for(int j = N/2 + 5; j <= N-2; j += 3) // electron hopping over orbitals
        {
            ampo += -J, "Cdag", j-2, "C", j;
            ampo += -J, "Cdag", j,   "C", j-2;
            ampo += -J, "Cdag", j+1, "C", j;
    ...
    for (int j = N/2+4 ; j <= N ; j += 3) //onsite V 这里几项我尝试配做particle-hole对称的形式，去掉了一个常数项
        {
            ampo += V, "Cdag", j, "C", j;
    }

    for (int j = N/2 +3; j <= N-1 ; j += 3) //onsite V
        {
            ampo += -V, "Cdag", j, "C", j;
    }
    
    // electron pairing
    for (int j = N/2 + 2 ; j <= N-3 ; j += 1) //electron pairing
        {
            ampo += D, "Cdag", j+3, "Cdag", j;
            ampo += D, "C", j,   "C", j+3;
    }
    ...
    //MIDDLE
    ampo += -t, "Cdag", N/2 , "C", N/2 + 4;
    ampo += -t, "Cdag", N/2 + 4, "C", N/2 ;
    ampo += -t, "Cdag", N/2 + 1, "C", N/2 + 3;
    ampo += -t, "Cdag", N/2 + 3, "C", N/2 + 1;
    ampo += D,  "Cdag", N/2 + 4, "Cdag", N/2 ;
    ampo += D, "C", N/2 ,   "C", N/2 + 4;
    ampo += D, "Cdag", N/2 + 3, "Cdag", N/2 + 1;
    ampo += D, "C", N/2 + 1,   "C", N/2 + 3;
    ampo += -J, "Cdag", N/2, "C", N/2 +2;
    ampo += -J, "Cdag", N/2 +2,   "C", N/2;
    ampo += -J, "Cdag", N/2+3, "C", N/2 +2;
    ampo += -J, "Cdag", N/2 +2,   "C", N/2+3;
    ampo += -J, "Cdag", N/2+1, "C", N/2 +2;
    ampo += -J, "Cdag", N/2 +2,   "C", N/2+1;
    ampo += -J, "Cdag", N/2+4, "C", N/2 +2;
    ampo += -J, "Cdag", N/2+2,   "C", N/2+4;
   ...
   std::string filename = "outputpsi/odd_symmetry_N_" 
    ...
    std::ofstream outfile5("/mnt/d/OneDrive/programs/vscodepython/marjorana/entanglement/try7");
    ...
    std::ofstream outfile6("/mnt/d/OneDrive/programs/vscodepython/marjorana/entanglement/try8");
    ...
    std::ofstream outfile7("/mnt/d/OneDrive/programs/vscodepython/marjorana/entanglement/try9");
