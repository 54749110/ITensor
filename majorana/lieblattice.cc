...
main()
   ...
   for(int j = 1; j <= N/3; ++j)
        {
        auto Q =  -3.14159265358+ 2*j*3.14159265358*3/N ;
        printfln("Q: ", Q);
        std::complex<double> paircorrelation = 0.0 ; 
        for(int s = 1; s <= N; ++s)
           {
            auto ampo3 = AutoMPO(sites) ;
            ampo3 += 1 , "Cdag", s, "Cdag", s;
            G1 = toMPO(ampo3);
            auto wave1 = inner(psi , G1 , psi) ;
            printfln("s: ", wave1);
            std::complex<double> wave2 = std::exp(std::complex<double>(0, -Q*s))*wave1;
            paircorrelation += wave2 ;
        }
        printfln("paircorrelation: ", j, paircorrelation);
    }
