...
main()
    ...
    //
    auto sites = Electron(N,{"ConserveSz",false}); //make a chain of N spin 1/2's
    ...
    for(auto j : range1(N-3))
        {
        ampo += 0.5*V,"S+",1,"Cdagup",j+1,"Cdn",j+2;
        ampo += 0.5*V,"S+",1,"Cdagdn",j+1,"Cup",j+2;
	ampo += 0.5*V,"S+",N,"Cdagup",j+1,"Cdn",j+2;
	ampo += 0.5*V,"S+",N,"Cdagdn",j+1,"Cup",j+2;
	ampo +=-0.5*V,"S+",1,"Cdagup",j+1,"Cdn",j+2;
        ampo +=-0.5*V,"S+",1,"Cdagdn",j+1,"Cup",j+2;
        ampo +=-0.5*V,"S+",N,"Cdagup",j+1,"Cdn",j+2;
        ampo +=-0.5*V,"S+",N,"Cdagdn",j+1,"Cup",j+2;
	ampo += 0.5*V,"S-",1,"Cdagup",j+1,"Cdn",j+2;
        ...
        ampo += 0.5*V,"S-",N,"Cdagup",j+1,"Cdn",j+2;
        ampo += 0.5*V,"S-",N,"Cdagdn",j+1,"Cup",j+2;
	ampo += V,"Sz",1,"Cdagup",j+1,"Cup",j+2;
        ampo +=-V,"Sz",1,"Cdagdn",j+1,"Cdn",j+2;
        ampo += V,"Sz",N,"Cdagup",j+1,"Cup",j+2;
        ampo +=-V,"Sz",N,"Cdagdn",j+1,"Cdn",j+2;
	ampo += 1,"Cdagup",j+1,"Cdn",j+2;
	ampo += 1,"Cdagdn",j+1,"Cup",j+2;
