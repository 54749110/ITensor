...
template<typename ASiteType, typename BSiteType, typename CSiteType>
class kondolattice : public SiteSet
    {
    public:
    ...
    kondolattice(IndexSet const& is)
        {
        int N = is.length();
        auto sites = SiteStore(N);
        for(auto j : range1(N))
...
template<typename ASiteType, typename BSiteType>
class twochannelkondosimple : public SiteSet
    {
    public:

    twochannelkondosimple() { }

    twochannelkondosimple(int N, 
                 Args const& args = Args::global())
        {
        auto sites = SiteStore(N);
        for(int j = 1; j <= N; ++j)
            {
            if(j != (N+1)/2) sites.set(j,ASiteType({args,"SiteNumber=",j}));
            else         sites.set(j,BSiteType({args,"SiteNumber=",j}));
            }
        SiteSet::init(std::move(sites));
        }

    twochannelkondosimple(IndexSet const& is)
        {
        int N = is.length();
        auto sites = SiteStore(N);
        for(auto j : range1(N))
            {
            if(j != (N+1)/2) sites.set(j,ASiteType(is(j)));
            else         sites.set(j,BSiteType(is(j)));
            }
        SiteSet::init(std::move(sites));
        }
    ...
    read(std::istream& s)
        ...
        if(N > 0)
            ...
            for(int j = 1; j <= N; ++j) 
                ...
                auto I = Index{};
                I.read(s);
                if(j != (N+1)/2) store.set(j,ASiteType(I));
                else       store.set(j,BSiteType(I));
                }
            init(std::move(store));
            }
...
template<typename ASiteType, typename BSiteType>
class twochannelkondosimpletwo : public SiteSet
    {
    public:

    twochannelkondosimpletwo() { }

    twochannelkondosimpletwo(int N, 
                 Args const& args = Args::global())
        {
        auto sites = SiteStore(N);
        for(int j = 1; j <= N; ++j)
            {
            if(j != N) sites.set(j,ASiteType({args,"SiteNumber=",j}));
            else         sites.set(j,BSiteType({args,"SiteNumber=",j}));
            }
        SiteSet::init(std::move(sites));
        }

    twochannelkondosimpletwo(IndexSet const& is)
        {
        int N = is.length();
        auto sites = SiteStore(N);
        for(auto j : range1(N))
            {
            if(j != N) sites.set(j,ASiteType(is(j)));
            else         sites.set(j,BSiteType(is(j)));
            }
        SiteSet::init(std::move(sites));
        }
    ...
    read(std::istream& s)
        ...
        if(N > 0)
            ...
            for(int j = 1; j <= N; ++j) 
                ...
                auto I = Index{};
                I.read(s);
                if(j != N) store.set(j,ASiteType(I));
                else       store.set(j,BSiteType(I));
                }
            init(std::move(store));
            }
