...
// N must be 3Z
   ...
   auto t= 1;
   auto Jk=1;
    //  for( int cycle= 1 ;cycle <= 100 ; cycle += 1 )  
    // {
    //   Jk += 0.1 ;
    for (int x=2 ; x<= (N-5)/2 ; x=x+2)
       {
    ...
    //     ampo +=       Jk, "Sz", N, "Sz", N-1;
      ...
      for(int j = 1; j <= N-1; j += 1) //2channel kondo hopping 
    {
        ampo += 0.5 * Jk, "S+", N, "S-", j;
        ampo += 0.5 * Jk, "S-", N, "S+", j;
        ampo +=       Jk, "Sz", N, "Sz", j;
    ...
    for (int j = 1; j <= (N-2*x-3)/2 ; j += 1) //electron hopping channel 1 part 1
    {
        ampo += -t, "Cdagup", j+1, "Cup", j;
        ampo += -t, "Cdagup", j,   "Cup", j+1;
        ampo += -t, "Cdagdn", j+1, "Cdn", j;
       ...
       // electron hopping channel 1 part 2
        ampo += -t, "Cdagup", N-2*x, "Cup", (N-2*x-1)/2 ;
        ampo += -t, "Cdagup", (N-2*x-1)/2,   "Cup", N-2*x;
        ampo += -t, "Cdagdn", N-2*x, "Cdn", (N-2*x-1)/2;
        ampo += -t, "Cdagdn", (N-2*x-1)/2,   "Cdn",N-2*x;
    for (int j=N-2*x ; j<=N-x-2 ; ++j)  //electron hopping channel 1 part 3
    {
        ampo += -t, "Cdagup", j+1, "Cup", j;
        ampo += -t, "Cdagup", j,   "Cup", j+1;
        ampo += -t, "Cdagdn", j+1, "Cdn", j;
     ...
     for (int j = (N+1-2*x)/2 ; j <= N-2*x-2 ; j += 1) //electron hopping channel 2 part 1
    {
        ampo += -t, "Cdagup", j+1, "Cup", j;
        ampo += -t, "Cdagup", j,   "Cup", j+1;
        ampo += -t, "Cdagdn", j+1, "Cdn", j;
        ...
        // electron hopping channel 2 part 2
        ampo += -t, "Cdagup", N-x, "Cup", N-2*x-1 ;
        ampo += -t, "Cdagup", N-2*x-1 ,   "Cup",N-x;
        ampo += -t, "Cdagdn", N-x, "Cdn", N-2*x-1 ;
        ampo += -t, "Cdagdn", N-2*x-1 ,   "Cdn",N-x;
    for (int j=N-x ; j<=N-2 ; ++j)  //electron hopping channel 2 part 3
    {
        ampo += -t, "Cdagup", j+1, "Cup", j;
        ampo += -t, "Cdagup", j,   "Cup", j+1;
        ampo += -t, "Cdagdn", j+1, "Cdn", j;
    ...
    // pbc
       ampo += -t, "Cdagup", N-1, "Cup", (N+1-2*x)/2;
       ampo += -t, "Cdagup", (N+1-2*x)/2,   "Cup", N-1;
       ampo += -t, "Cdagdn", N-1, "Cdn", (N+1-2*x)/2;
       ampo += -t, "Cdagdn", (N+1-2*x)/2 ,   "Cdn", N-1; 
       ampo += -t, "Cdagup", N-x-1, "Cup", 1;
       ampo += -t, "Cdagup", 1,   "Cup", N-x-1;
       ampo += -t, "Cdagdn", N-x-1, "Cdn", 1;
       ampo += -t, "Cdagdn", 1,   "Cdn", N-x-1;
